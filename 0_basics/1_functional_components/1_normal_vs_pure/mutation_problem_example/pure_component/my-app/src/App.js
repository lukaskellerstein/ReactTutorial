...
const SomeRandomTextPure = memo(SomeRandomText);

const SomeListOfWords = (props) => {
  return <div>{props.words.join(",")}</div>;
};
const SomeListOfWordsPure = memo(SomeListOfWords);
...
const SomeDeepObjectPure = memo(SomeDeepObject);
...
const App = () => {
  ...
  return (
    <div>
      ...
      <SomeRandomTextPure text={myState.randomText} />
      <br />
      <button onClick={handleClick1.bind(this)}>Click here</button>
      <SomeListOfWordsPure words={myState.words} />
      <br />
      <button onClick={handleClick2.bind(this)}>Click here</button>
      <SomeDeepObjectPure someObject={myState.someObject} />
    </div>
  );
};
